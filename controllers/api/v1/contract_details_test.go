...
func TestSearchScrip(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/contractdetails/searchScrip", nil)
	...
	w = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/contractdetails/searchScrip", strings.NewReader("{\"key\": \"gyhgh\"}"))
	...
	//Init order provider
	InitContractDetailsProvider(contractDetailsMock{})
...
func TestScripInfo(t *testing.T) {
	...
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/contractdetails/scripInfo", nil)
	expected := "{\"status\":false,\"message\":\"INVALID REQUEST\",\"errorcode\":\"P11017\",\"data\":null}"
	...
	w = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/contractdetails/scripInfo", strings.NewReader("{\"exchange\": \"DSE\",\"info\": \"gkffd\",\"token\": \"fdssd\"}"))
	expected = "{\"status\":false,\"message\":\"INVALID DEVICE ID\",\"errorcode\":\"P11034\",\"data\":null}"
	tests = []struct {
		name string
		args args
	...
	reqHsuccess.DeviceType = "kugbkjbwVJKABWVLAWKJ"
	ctx.Set("reqH", reqHsuccess)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/contractdetails/scripInfo", strings.NewReader("{\"exchange\": \"NSE\",\"info\": \"gkffd\",\"token\": \"fdssd\"}"))
	expected = "{\"status\":true,\"message\":\"SUCCESS\",\"errorcode\":\"\",\"data\":null}"

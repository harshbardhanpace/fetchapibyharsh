...
func TestFetchDematHoldings(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/fetchDematHoldings", nil)
	...
	w = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/fetchDematHoldings", strings.NewReader("{\"clientId\":\"Client1\"}"))
	...
	//Init order provider
	InitPortfolioProvider(portfolioMock{})
...
func TestConvertPositions(t *testing.T) {
	...
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/convertPositions", nil)
	expected := "{\"status\":false,\"message\":\"INVALID REQUEST\",\"errorcode\":\"P11017\",\"data\":null}"
	tests := []struct {
		name string
		args args
	...
	w = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/convertPositions", strings.NewReader("{\"clientId\":\"Client1\",\"exchange\":\"NSE\",\"instrumentToken\":22,\"newProduct\":\"MIS\",\"orderSide\":\"BUY\",\"product\":\"CNC\",\"quantity\":1,\"validity\":\"DAY\"}"))
	expected = "{\"status\":false,\"message\":\"INVALID DEVICE ID\",\"errorcode\":\"P11034\",\"data\":null}"
	tests = []struct {
		name string
		args args
	...
	ctx.Set("reqH", reqH)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/convertPositions", strings.NewReader("{\"clientId\":\"Client1\",\"exchange\":\"DSE\",\"instrumentToken\":22,\"newProduct\":\"MIS\",\"orderSide\":\"BUY\",\"product\":\"CNC\",\"quantity\":1,\"validity\":\"DAY\"}"))
	expected = "{\"status\":false,\"message\":\"INVALID REQUEST\",\"errorcode\":\"P11017\",\"data\":null}"
	tests = []struct {
		name string
		args args
...
func TestGetPositions(t *testing.T) {
	...
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/getPositions", nil)
	expected := "{\"status\":false,\"message\":\"INVALID REQUEST\",\"errorcode\":\"P11017\",\"data\":null}"
	tests := []struct {
		name string
		args args
	...
	w = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(w)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/getPositions", strings.NewReader("{\"clientId\":\"Client1\",\"type\":\"live\"}"))
	expected = "{\"status\":false,\"message\":\"INVALID DEVICE ID\",\"errorcode\":\"P11034\",\"data\":null}"
	tests = []struct {
		name string
		args args
	...
	reqH.DeviceType = "kugbkjbwVJKABWVLAWKJ"
	ctx.Set("reqH", reqH)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/v1/portfolioapis/getPositions", strings.NewReader("{\"clientId\":\"Client1\",\"type\":\"lol\"}"))
	expected = "{\"status\":false,\"message\":\"INVALID REQUEST\",\"errorcode\":\"P11017\",\"data\":null}"
	tests = []struct {
		name string
		args args
